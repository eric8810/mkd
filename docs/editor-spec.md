...
| 测试 | ✅ | 106 个单元测试全绿、零警告；万行解析 0.04s |

### 未实现（P2 / 框架限制）

| 行为 | 原因 |
| ---- | ---- |
| 系统文件拖入打开 | GPUI 0.2 拖放 API 仅支持应用内自定义类型，无系统文件 URL 事件 |
| bidi（NAV-08） | 深层排版，P2 |
| HTML 粘贴转 markdown（CLP-05） | 需 html→md 转换依赖，P2 |
| VoiceOver / 拼写检查（ACC） | 依赖原生辅助功能集成，P2 |
| 自动保存（SAV-08） | 有意不做默认自动写盘，P2 可选 |
| 智能引号（IR-08） | 中文场景默认关闭 |
| 列表项拖拽排序（DRG-03） | 需块级拖拽模型，P2 |
| 撤销历史上限策略（UND-05） | 无界栈，P2 |
