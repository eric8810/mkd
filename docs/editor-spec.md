...
| 能力 | 现有 | 差距 |
| ---- | ---- | ---- |
| 光标/选区基础 | ✅ | 上下列需视觉列；缺拖选/双击选词/块级选择 |
| 字符/IME 输入 | ✅（EntityInputHandler） | 缺组合中编辑回归与候选窗实测 |
| 编辑操作 | ✅ | 需 Op 化以便测试 |
| 输入规则 | ❌ | 全缺（IR-01~12） |
| 撤销/重做 | ❌ | 全缺（UND-01~08） |
| 格式化命令 | ❌ | Cmd+B/I/K、块类型、选区转列表（FMT-04~11） |
| 列表续行/缩进 | 部分 | 回车续行、Tab 缩进、序号重排（LST-01~11） |
| **回车/段落语义** | ❌ 仅拆行 | **MD-01~08：段落空行分隔、Shift+Enter 硬换行均未定义** |
| 未保存退出保护 | ❌ | SAV-05~07 |
| 拖放 | ❌ | DRG-01~03 |
| 查找替换 | ❌ | FND-01 |
| 测试 | 27 单元 | 需 DSL + 不变量 + fixtures + 三层 |
